...
Elm.Console.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $LogLine = Elm.LogLine.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var consoleStyle = $Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                           ,_0: "font-family"
                                                           ,_1: "monospace"}
                                                          ,{ctor: "_Tuple2"
                                                           ,_0: "whitespace"
                                                           ,_1: "pre"}]));
   var Context = {_: {}};
   var updateLog = F2(function (newLog,
   model) {
      return _U.replace([["logLines"
                         ,newLog]],
      model);
   });
   ...
   model) {
      return function () {
         ...
         {case "ModifyLine":
            return function () {
                 var modifyLine = function (logLineModel) {
                    return _U.eq($LogLine.idOf(logLineModel),
                    action._0) ? A2($LogLine.update,
                    action._1,
                    logLineModel) : logLineModel;
                 };
                 return _U.replace([["logLines"
                                    ,A2($List.map,
                                    modifyLine,
   ...
   var ModifyLine = F2(function (a,
   b) {
      return {ctor: "ModifyLine"
             ,_0: a
             ,_1: b};
   });
   var viewLogLine = F2(function (address,
   logLineModel) {
      return function () {
         var forwardingAddress = A2($Signal.forwardTo,
         address,
         ModifyLine($LogLine.idOf(logLineModel)));
         return A2($LogLine.view,
         forwardingAddress,
         logLineModel);
      }();
   });
   var viewLog = F2(function (address,
   model) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("console-log")
                   ,consoleStyle]),
      A2($List.map,
      viewLogLine(address),
      model.logLines));
   ...
   model) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("console")]),
      _L.fromArray([A2(viewLog,
      address,
      model)]));
   });
   var NoOp = {ctor: "NoOp"};
   var init = {_: {}
              ,logLines: _L.fromArray([])};
   var Model = function (a) {
      return {_: {},logLines: a};
   };
   var initWithLogLines = function (logLines) {
      return Model(logLines);
   };
   _elm.Console.values = {_op: _op
                         ,init: init
                         ,initWithLogLines: initWithLogLines
                         ,update: update
                         ,updateLog: updateLog
                         ,view: view
                         ,Model: Model
...
Elm.CreeperPanel = Elm.CreeperPanel || {};
Elm.CreeperPanel.make = function (_elm) {
   "use strict";
   _elm.CreeperPanel = _elm.CreeperPanel || {};
   if (_elm.CreeperPanel.values)
   return _elm.CreeperPanel.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "CreeperPanel",
   ...
   model) {
      return function () {
         switch (action.ctor)
         {case "ModifyConsole":
            return _U.replace([["console"
                               ,A2($Console.update,
                               action._0,
                               model.console)]],
              model);
            case "UpdateConsoleLog":
            return _U.replace([["logLines"
                               ,action._0]
                              ,["console"
                               ,A2($Console.updateLog,
                               action._0,
                               model.console)]],
              model);}
         _U.badCase($moduleName,
   ...
   var ModifyConsole = function (a) {
      return {ctor: "ModifyConsole"
             ,_0: a};
   };
   var consoleView = F2(function (address,
   model) {
      return A2($Console.view,
      A2($Signal.forwardTo,
      address,
      ModifyConsole),
      model.console);
   ...
   model) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("creeperpanel")]),
      _L.fromArray([A2(consoleView,
      address,
      model)]));
   });
   var UpdateConsoleLog = function (a) {
      return {ctor: "UpdateConsoleLog"
             ,_0: a};
   };
   var init = function () {
      var logLines = $LogLine.listFromRaw("Here\'s a log line.\nHere\'s a second line.\nYay a third!\n");
      return {_: {}
   ...
   var main = $StartApp.start({_: {}
                              ...
                              ,update: update
                              ,view: view});
   _elm.CreeperPanel.values = {_op: _op
                              ,main: main
                              ,Model: Model
                              ,init: init
                              ,UpdateConsoleLog: UpdateConsoleLog
                              ,ModifyConsole: ModifyConsole
                              ,update: update
                              ,view: view
...
Elm.LogLine = Elm.LogLine || {};
Elm.LogLine.make = function (_elm) {
   "use strict";
   _elm.LogLine = _elm.LogLine || {};
   if (_elm.LogLine.values)
   return _elm.LogLine.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   model) {
      return A2($Html.p,
      _L.fromArray([$Html$Attributes.$class("log-line")]),
   ...
   var listFromRaw = function (rawLog) {
      return function () {
         var rawLines = $List.filter(function ($) {
            return $Basics.not($String.isEmpty($));
         })($List.map($String.trim)($String.lines(rawLog)));
         return A2($List.indexedMap,
         F2(function (tid,text) {
            return A2(Model,tid,text);
         }),
         rawLines);
      }();
   };
   _elm.LogLine.values = {_op: _op
                         ,listFromRaw: listFromRaw
                         ,idOf: idOf
                         ,update: update
                         ,view: view
                         ,Model: Model};
